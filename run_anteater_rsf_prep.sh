...
if [ -f results/anteater_rsf_results_final4.csv ]; then
        echo "Results file already exists! continuing..."
else
        echo "creating results file"
        echo "aid,nf_est,nf_lcl,nf_ucl,pf_est,pf_lcl,pf_ucl,nf_temp_est,nf_temp_lcl,nf_temp_ucl,pf_temp_est,pf_temp_lcl,pf_temp_ucl" > results/anteater_rsf_results_final4.csv
fi

Rscript anteater_rsfs_prep.R ${list[SLURM_ARRAY_TASK_ID]}     # name of script
