...
if [ -f results/anteater_rsf_results_2.csv ]; then
	echo "Results file already exists! continuing..."
else
	echo "creating results file anteater_rsf_results_2.csv"
	echo "aid,pasture_est,pasture_lcl,pasture_ucl,nf_est,nf_lcl,nf_ucl,pf_est,pf_lcl,pf_ucl,stream_est,stream_lcl,stream_ucl,area,area_lcl,area_ucl,pasture_temp_est,pasture_temp_lcl,pasture_temp_ucl,nf_temp_est,nf_temp_lcl,nf_temp_ucl,pf_temp_est,pf_temp_lcl,pf_temp_ucl,stream_temp_est,stream_temp_lcl,stream_temp_ucl,runtime" > results/anteater_rsf_results_2.csv
fi
